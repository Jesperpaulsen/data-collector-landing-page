...
export default class SanityPageService<T> {
  ...
  getPreviewHook(initialData: T & BaseDataType, params = {}) {
    const subscription = {
      initialData,
      enabled: initialData.preview,
      params: { slug: "" } as { slug?: string },
    };
    subscription.params = params;
    if (initialData.slug?.length > 0)
      subscription.params.slug = initialData.slug;
  ...
  async fetchQuery({
    ...
    params?: any;
    preview?: boolean;
  }): Promise<{ props: T & BaseDataType; notFound: boolean }> {
...
function injectQuery(originalQuery: string, queryToInject: string) {
  ...
  if (injectIdx <= 1 || originalQuery[injectIdx] !== "}")
    throw new Error(
      `Something went wrong when injecting query ${queryToInject} into ${originalQuery}`
    );
  if (originalQuery[injectIdx - 2] === ",")
    throw new Error(
      `Don't add trailing comma to the last element in the query when using inject body. Query with trailing comma is ${originalQuery}`
    );
